...
const canEditProduct = require('../middleware/productAuthorization');
...
router.delete('/:id', authMiddleWare, canEditProduct, async (req, res) => {
  const {id} = req.params;
  const deletedProuct = await productsController.deleteProduct(id);
  res.json(deletedProuct);
});
router.patch('/:id', authMiddleWare, canEditProduct, async (req, res) => {
  const {id} = req.params;
  const {body} = req;
  const updatedProduct = await productsController.update(id, body);
  res.json(updatedProduct);
});
router.patch('/:id/stock', authMiddleWare, canEditProduct, async (req, res) => {
  const {id} = req.params;
